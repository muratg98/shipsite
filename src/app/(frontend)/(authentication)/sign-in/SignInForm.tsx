...
export default function SignInForm({ authMethods, media }: { authMethods: string[], media: MediaType }) {
  ...
  const handleCredentialsSignIn = async (values: z.infer<typeof signInSchema>) => {
    ...
    await authClient.signIn.email(
      { email: values.email, password: values.password, callbackURL: callbackURL },
      {
  ...
  return (
    <div className="grow flex flex-col items-center justify-center p-4 bg-slate-50 py-32">
      {media && (
        <a href="/" className="mb-8"><Media resource={media} className="w-32 mx-auto"/></a>
      )}
      <Card className="w-full max-w-md bg-white shadow-xl">
        ...
        <CardContent>
          {authMethods.includes("credentials") || authMethods.includes("magicLink") ? (
            <Form {...form}>
              ...
              >
                ...
                {showPasswordField && (
                  <LoadingButton pending={!!pending.credentials}>Sign in</LoadingButton>
                )}
                ...
                >
                    {showPasswordField ? "Sign in with Magic Link" : "Sign in with Password"}
                </button>
                )}
              </form>
            </Form>
          ...
          <div className="mt-4 text-center text-sm flex justify-center gap-x-2">
